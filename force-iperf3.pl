#!/usr/bin/perl -T
# Allow ssh to run any iperf3 client, but only iperf3.
...
# filter out server and file access options
my %opts;
@ARGV = @args;
shift;
getopts('p:f:i:A:B:V:J:d:v:h:c:ub:t:n:k:l:P:Rw:M:N46S:L:Z:O:T:C:', \%opts)
